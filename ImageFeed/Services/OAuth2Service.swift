...
final class OAuth2Service {
    ...
    func fetchAuthToken(_ code: String, handler: @escaping (Result<String, Error>) -> Void) {
        ...
        task = networkClient.fetch(requestType: .urlRequest(urlRequest: request), withToken: false) {
    ...
    private func makeRequest(code: String) -> URLRequest? {
        if var urlComponents = URLComponents(string: AuthConfiguration.tokenURL) {
